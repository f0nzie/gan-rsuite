---
title: "Test lapply on epochs and discriminator"
...
```{r}
y <- 0
invisible(lapply(seq(0, 10), function(x) {
    y = y + 1
    cat(y)
}))
```
...
```{r}
do.call(c, lapply(1:10, foo_outer))
```

```{r}
# change for-loops to several lapply functions
library(rTorch)
...
train <- function() {
    ...
    lapply(1:num_epochs, function(epoch) {
        cat(sprintf("%5d:", epoch))
        # for (d_index in (1:d_steps)) {
        lapply(1:d_steps, function(x) {
