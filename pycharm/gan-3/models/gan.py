...
class GAN(object):
    def __init__(self, args):
        ...
        print(strtobool(args.cuda))
        self.check_cuda(strtobool(args.cuda))   # fixed
    ...
    def train(self, train_loader):
        ...
        for epoch in range(self.epochs+1):
            for i, (images, _) in enumerate(train_loader):
                ...
                if self.cuda:
                    real_labels = Variable(torch.ones(self.batch_size)).cuda(self.cuda_index)  # fixed
                    fake_labels = Variable(torch.zeros(self.batch_size)).cuda(self.cuda_index)  # fixed
                ...
                if ((i + 1) % 100) == 0:
                    print("Epoch: [%2d] [%4d/%4d] %d" %
                          ((epoch + 1), (i + 1), train_loader.dataset.__len__(), self.batch_size))
                    print("D_loss: %.8f, G_loss: %.8f" % (d_loss.item(), g_loss.item()))
